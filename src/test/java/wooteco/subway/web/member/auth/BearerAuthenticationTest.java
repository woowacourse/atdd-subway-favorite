...
class BearerAuthenticationTest {
    ...
    void setAuthentication() {
        BearerAuthentication bearerAuthentication = new BearerAuthentication((request, tokenProvider) -> "token", new JwtTokenProvider("1", 2));
    ...
    void getAuthentication() {
        BearerAuthentication bearerAuthentication = new BearerAuthentication(new AuthorizationExtractor(), new JwtTokenProvider("1", 1));

        NativeWebRequest nativeWebRequest = new ServletWebRequest(new MockHttpServletRequest());
        assertThatThrownBy(() -> bearerAuthentication.getAuthentication(nativeWebRequest, String.class))
                .isInstanceOf(InvalidAuthenticationException.class)
