...
public class FavoriteAcceptanceTest extends AcceptanceTest {
    ...
    void manageFavorite() {
        ...
        //when 로그인하고 발급받은 토큰으로 즐겨찾기를 추가한다.
        ...
        //when 로그인하고 발급받은 토큰으로 즐겨찾기를 삭제한다.
        deleteFavorite(token, 1L);
        deleteFavorite(token, 3L);

        //then 제대로 삭제됐는지 확인한다.
        List<FavoriteResponse> favoriteResponsesAfterDelete = getOwnFavorites(token);
        assertThat(favoriteResponsesAfterDelete).hasSize(1);
        assertThat(favoriteResponsesAfterDelete.get(0).getId()).isEqualTo(2L);
    }
}
