...
public class FavoriteDocumentation {
    ...
    public static RestDocumentationResultHandler getFavorite() {
        return document("favorite/get",
            ...
            responseFields(
                fieldWithPath("favoriteResponses[].id").type(JsonFieldType.NUMBER)
                    .description("The persist favorite id"),
                fieldWithPath("favoriteResponses[].source").type(JsonFieldType.STRING)
                    .description("The source station name"),
                fieldWithPath("favoriteResponses[].target").type(JsonFieldType.STRING)
                    .description("The target station name")
            )
        );
    }
