...
public class PathAcceptanceTest extends AcceptanceTest {
    ...
    public void findPathByDistance() {
        PathResponse pathResponse = findPath(강남역, STATION_NAME_DOGOK,
            "DISTANCE");
        List<StationResponse> path = pathResponse.getStations();
        assertThat(path).hasSize(5);
        assertThat(path.get(0).getName()).isEqualTo(강남역);
        assertThat(path.get(1).getName()).isEqualTo(역삼역);
        assertThat(path.get(2).getName()).isEqualTo(선릉역);
