    ...
    Stream<DynamicTest> manageMember() {
        ...
        return Stream.of(
                DynamicTest.dynamicTest("유저 인가 실패(토큰 존재 x)", () -> {
                    ...
                    // then 유저 정보 조회가 실패했다
                    TokenResponse emptyTokenResponse = new TokenResponse("", "");
                    failToGetMemberByAuthentication(TEST_USER_EMAIL, sessionId, emptyTokenResponse);
                }),
                DynamicTest.dynamicTest("유저 인가 실패(잘못된 토큰)", () -> {
                    // when 다른 유저의 토큰으로 정보 조회를 한다
                    // then 유저 정보 조회가 실패했다
                    failToGetMemberByAuthentication(TEST_USER_EMAIL, sessionId, tokenResponse2);
                }),
                DynamicTest.dynamicTest("유저 인가 실패(세션 존재 x)", () -> {
                    // when 세션 없이 유저 정보를 조회한다
                    // then 유저 정보 조회가 실패했다
                    failToGetMemberByAuthentication(TEST_USER_EMAIL, "", tokenResponse2);
                }),
                DynamicTest.dynamicTest("유저 인가 실패(잘못된 세션)", () -> {
                    // when 다른 유저의 세션Id로 정보 조회를 한다
                    // then 유저 정보 조회가 실패했다
                    failToGetMemberByAuthentication(TEST_USER_EMAIL, sessionId2, tokenResponse);
                }),
                DynamicTest.dynamicTest("유저 정보 조회", () -> {
                    // when 유저 정보 조회를 한다
                    MemberResponse memberResponse = getMember(TEST_USER_EMAIL, sessionId, tokenResponse);
                ...
                DynamicTest.dynamicTest("유저 정보 수정", () -> {
                    ...
                    updateMember(memberId, sessionId, tokenResponse, newName, newPassword);
                ...
                DynamicTest.dynamicTest("유저 정보 삭제", () -> {
                    // when 유저 정보를 삭제한다
                    deleteMember(memberId, sessionId, tokenResponse);

                    // then 유저 정보가 삭제되었다
                    failToGetMemberByNotExisting(memberId, sessionId, tokenResponse);
                })
        );
    }
}
