...
public class FavoriteAcceptanceTest extends AcceptanceTest {
	...
	void manageFavorite() {
		...
		FavoriteExistenceResponse favoriteExistenceResponse = existFavorite(loginToken,
			stationKangnam.getId(),
			stationHanti.getId());
		assertThat(favoriteExistenceResponse.isExistence()).isFalse();

		addFavorite(loginToken, stationYeoksam.getId(), stationHanti.getId());
		favoriteExistenceResponse = existFavorite(loginToken,
			stationYeoksam.getId(), stationHanti.getId());
		assertThat(favoriteExistenceResponse.isExistence()).isTrue();

		removeFavorite(loginToken, stationYeoksam.getId(), stationHanti.getId());
		favoriteExistenceResponse = existFavorite(loginToken, stationYeoksam.getId(),
			stationHanti.getId());
		assertThat(favoriteExistenceResponse.isExistence()).isFalse();
	...
	private FavoriteExistenceResponse existFavorite(TokenResponse loginToken, Long sourceStationId,
		...
		return
			given().
				...
				when().
				get("/me/favorites/source/{sourceId}/target/{targetId}/existsPath", sourceStationId,
					targetStationId).
				then().
				log().all().
	...
	private void removeFavorite(TokenResponse loginToken, Long sourceStationId,
		Long targetStationId) {
		given().
			accept(MediaType.APPLICATION_JSON_VALUE).
			auth().
			oauth2(loginToken.getAccessToken()).
			when().
			delete("/me/favorites/source/{sourceId}/target/{targetId}", sourceStationId,
				targetStationId).
			then().
			log().all().
