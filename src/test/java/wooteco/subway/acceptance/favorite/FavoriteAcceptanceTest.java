...
public class FavoriteAcceptanceTest extends AcceptanceTest {
	...
	void favorite() {
		...
		// when : 즐겨찾기를 추가한다.
		createFavorite(tokenResponse, STATION_NAME_KANGNAM, STATION_NAME_SEOLLEUNG);

		// then : 즐겨찾기가 추가되었다.
		FavoriteResponse favoriteResponse = getFavorites(tokenResponse).get(0);
		assertThat(favoriteResponse.getPreStationName()).isEqualTo(STATION_NAME_KANGNAM);
		assertThat(favoriteResponse.getStationName()).isEqualTo(STATION_NAME_SEOLLEUNG);
