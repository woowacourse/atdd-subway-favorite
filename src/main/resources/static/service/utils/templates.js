...
export const edgeItemTemplate = edge => {
  return `<li data-edge-id="${edge.id}" class="edge-item w-full border border-gray-300 py-2 px-3 text-left text-gray-700">
            ...
            <span data-source-station-id="${edge.sourceStationId}">${edge.sourceStationName ? edge.sourceStationName : "출발역"}</span>
            <span class="mdi mdi-arrow-right text-gray-500"></span>
            <span data-target-station-id="${edge.targetStationId}">${edge.targetStationName ? edge.targetStationName : "도착역"}</span>
