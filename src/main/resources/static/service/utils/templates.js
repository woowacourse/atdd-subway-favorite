...
export const edgeItemTemplate = edge => {
    return `<li data-edge-id="${
        edge.id
            ...
            <span data-source-station-id="${edge.source.id}">${
        edge.source.name ? edge.source.name : "출발역"
    }</span>
            <span class="mdi mdi-arrow-right text-gray-500"></span>
            <span data-target-station-id="${edge.target.id}">${
        edge.target.name ? edge.target.name : "도착역"
    }</span>
