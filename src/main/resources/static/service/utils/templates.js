...
export const memberInfo = member => {
  ...
  return `<div class="dropdown inline-block absolute right-10px">
      ...
      <ul class="dropdown-menu absolute hidden text-gray-700 bg-white shadow rounded z-10">
        ...
        <li>
          <a href="/logout" class="rounded-b hover:bg-gray-400 py-3 px-5 block whitespace-no-wrap" id="logout">
...
class="edge-item w-full border border-gray-300 py-2 px-3 text-left text-gray-700">
            ...
            <span data-source-station-id="${edge.sourceStation.id}">${
    edge.sourceStation.name ? edge.sourceStation.name : "출발역"
  }</span>
            <span class="mdi mdi-arrow-right text-gray-500"></span>
            <span data-target-station-id="${edge.targetStation.id}">${
    edge.targetStation.name ? edge.targetStation.name : "도착역"
  }</span>
