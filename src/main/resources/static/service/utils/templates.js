...
export const memberInfo = member => {
    ...
    return `<div class="dropdown inline-block absolute right-10px">
      ...
      <ul class="dropdown-menu absolute hidden text-gray-700 bg-white shadow rounded z-10">
        ...
        <li>
          <a href="/" id="logout" class="rounded-b hover:bg-gray-400 py-3 px-5 block whitespace-no-wrap">
...
export const edgeItemTemplate = edge => {
    return `<li data-depart-station-id="${edge.departStationId}"
                data-arrive-station-id="${edge.arriveStationId}"
            ...
            <span>${
        edge.departStationName ? edge.departStationName : "출발역"
    }</span>
            <span class="mdi mdi-arrow-right text-gray-500"></span>
            <span>${
        edge.arriveStationName ? edge.arriveStationName : "도착역"
    }</span>
