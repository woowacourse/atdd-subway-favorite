...
public class PathService {
    ...
    public PathResponse findPath(String source, String target, PathType type) {
        ...
        List<Long> ids = stationRepository.findIdsByNames(Arrays.asList(source, target));
        Long sourceId = ids.get(0);
        Long targetId = ids.get(1);

        List<Long> path = graphService.findPath(lines, sourceId, targetId, type);
